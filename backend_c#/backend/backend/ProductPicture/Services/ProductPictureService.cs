...
namespace backend.Producer.Services {
    public class ProductPictureService : IProductPictureService {
        ...
        public ProductPictureService(IAmazonS3 amazonS3, string bucketName) {
            _amazonS3 = amazonS3;
            ...
            try {
                ...
                foreach (var picture in product.Pictures) {
                    var getObjectRequest = new GetPreSignedUrlRequest {
                        BucketName = _BucketName,
                        Key = $"{product.ProducerId}/products/{product.Id}/{picture.Id}",
        ...
        public async Task<List<PutObjectResponse>> UploadImageAsync(List<CreateProductPictureDTO> pictures, Models.Product product) {
